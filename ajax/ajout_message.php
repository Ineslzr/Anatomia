...
if(isset($_SESSION['nomUtilisateur'])&& !empty($_POST['message'])) {
	$auteur=$_SESSION['nomUtilisateur'];
	$message=$_POST['message'];
	$idSujet=$_POST['idSujet'];
	$parent=$_POST['parent'];
	$parent_id=$_POST['parent_id'];
	$message_encoder=remove_script_balises($message);

	$prepare = $bdd->prepare("INSERT INTO message(auteur_message,message,parent,parent_id,idSujet) VALUES(?,?,?,?,?);");
	$prepare-> execute(array($auteur,$message_encoder,$parent,$parent_id,$idSujet));
	...
	$tab = array ("auteur" => $auteur, "message"=> $message_encoder, "date"=>$resultat->fetch()['date_message'], "id"=> $lastId);
